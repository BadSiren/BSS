...
namespace BSS.Input {
    public class BaseInput : SerializedMonoBehaviour {
        ...
        public void onClickInMousePoint(EClickType _eClickType) {
            var clickableList = getClickableList().FindAll(x => x.eClickType == _eClickType);
        ...
        IEnumerator coWaitDoubleTouch() {
            ...
            while (isTouching) {
                if (UnityEngine.Input.GetMouseButtonDown(0)) {
                    isTouching = false;
                    onClickInMousePoint(EClickType.Double);
                    break;
                }
                if (Time.time - lastTime > doubleInterval) {
                    isTouching = false;
                    onClickInMousePoint(EClickType.Once);
                    break;
                }
                yield return null;
            }
